...
import { fabricGif } from '../objects/StaticGif'
...
class ObjectToFabric {
  ...
  async run(item, options, inGroup = false) {
    ...
    switch (item.type) {
      ...
      case ObjectType.STATIC_VIDEO:
        object = await this[ObjectType.STATIC_VIDEO](item, options, inGroup)
        break
      ...
      case ObjectType.STATIC_AUDIO:
        object = await this[ObjectType.STATIC_AUDIO](item, options, inGroup)
        break
      case ObjectType.STATIC_GIF:
        object = await this[ObjectType.STATIC_GIF](item, options, inGroup)
    }
    return object
  }
  ...
  async [ObjectType.STATIC_GIF](item, options, inGroup) {
    return new Promise(async (resolve, reject) => {
      try {
        const baseOptions = this.getBaseOptions(item, options, inGroup)
        const src = item.metadata.src

        const gif = await fabricGif(src, { ...baseOptions, src });

        // @ts-ignore
        gif.type = ObjectType.STATIC_GIF
  ...
  [ObjectType.STATIC_VIDEO](item, options, inGroup) {
    return new Promise(async (resolve, reject) => {
      try {
        const baseOptions = this.getBaseOptions(item, options, inGroup)
        const src = item.metadata.src
        const video = await this.editor.manager.videos.loadVideo(src)
        resolve(video)
      } catch (err) {
        reject(err)
      }
  ...
  [ObjectType.STATIC_AUDIO](item, options, inGroup) {
    return new Promise(async (resolve, reject) => {
      try {
        const baseOptions = this.getBaseOptions(item, options, inGroup)
        const { src, totalDuration } = item.metadata;
